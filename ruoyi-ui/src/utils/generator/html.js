...
let confGlobal, someSpanIsNot24
...
export const vueScript = cssStr => {
  return `
    <style>${cssStr}</style>
  `
}

const buildFormTemplate = (conf, child, type) => {
  let labelPosition = ''
  if (conf.labelPosition !== 'right') {
    labelPosition = `label-position="${conf.labelPosition}"`
  }
  const disabled = conf.disabled ? `:disable="${conf.disabled}"` : ''
  let str = `
    <ElForm ref="${conf.formRef}" :model="${conf.formModel}" :rules="${conf.formRules}">
      ${child}
      ${buildFormTemplate(conf, type)}
    </ElForm>`
  if (someSpanIsNot24) {
    str = `<ElRow :gutter="${conf.gutter}">${str}</ElRow>`
  }
  return str
}


const colWrapper = (element, str) => {
  if (someSpanIsNot24 || element.span !== 24) {
    return `
      <ElCol :span="${element.span}">${str}</ElCol>
    `
    return str
  }
}
